...
import {CollectionInfos /*JsonSchemaProperty*/} from '../types/mongodb-custom';
...
(async () =>
{
    ...
    const mongodbCollectionsOrder: { [s: string]: number } = {
        categories: 0,
        divisions: 1,
        leagueSeasons: 1,
        matchTypes: 0,
    ...
    const mongodbCollections: CollectionInfos[] = (await mongodbClient.listCollections().toArray()).sort((collectionA, collectionB) =>
    {
        const orderGet: (order?: number) => number = (order: number = 2) =>
        {
            return order;
        };
    ...
    await pgClient.query(
        pgFormat(`SELECT truncate_tables(%L);`, process.env.NODE_ENV === `production` ? `admin` : `postgres`)
    );

    const _idsIds: Map<string, number> = new Map();
    ...
    await Promise.all(mongodbCollections.map(async (collection) =>
    {
        ...
        {
            result.forEach((row: { _id: Mongodb.ObjectId, [s: string]: any }, index: number) =>
            {
...
1
    ...
    await mongodbCollections.reduce((a: Promise<any>, collection: CollectionInfos) =>
    {
        return a.then(async () =>
        {
            const {$jsonSchema} = collection.options.validator;
            const tableName: string = collection.name === `matchTypes` ? `matches_types` : new VueString(collection.name).caseSnakeTo().toString();
            ...
            const columnsOrder: { [s: string]: number } = {
                created: 999,
                id: 0,
                _id: 1,
                updated: 1000
            ...
            const columnsCompareFn: (columnA: string, columnB: string) => number = (propA: string, propB: string) =>
            {
            ...
            const columns: string[] = [...Object.keys($jsonSchema.properties), `created`].sort(columnsCompareFn);

            const insertColumns: string[] = columns.filter((columnName) =>
            {
                return columnName !== `_id` && columnName !== `valid` && !(columnName === `matchType_id` && [`leagueRegistrations`, `leagueTableRecords`].includes(
                    collection.name));
            });

            const rows: { _id: Mongodb.ObjectId }[] = await mongodbClient.collection(collection.name).find({}).toArray();
            ...
            }).map((row) =>
            {
                ...
                {
                    const column: any = row[columnName];
                    ...
                    switch ($jsonSchema.properties[columnName].bsonType)
                    {
                        case 'string':
                            return `'${column}'`;
                        case `date`:
                            return `to_timestamp(${(column as Date).valueOf()} / 1000.0)`;
                        case `array`:
                        ...
                        case `objectId`:
                            return _idsIds.get((column as Mongodb.ObjectId).toHexString());
                        default:
                            return column;
                    }
            ...
            const insertQuery: string = `INSERT INTO ${tableName}(${insertColumnsString}) VALUES${values} ON CONFLICT DO NOTHING;`;
